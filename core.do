...
*Import & pare down to select vars; adjust as you need
	...
	*Late 90s		
		import delimited "${project}\ic98hdac_data_stata.csv", clear
		...
		save "${project}\small1998_directory.dta", replace	
	
		import delimited "${project}\ic99_hd_data_stata.csv", clear
			keep unitid instnm addr city stabbr zip fips sector hbcu closedat
			foreach var in unitid instnm addr city stabbr zip fips sector hbcu closedat {
				tostring `var', replace
			}
		save "${project}\small1999_directory.dta", replace
		
	*Early 00s
	foreach year of numlist 2000/2001 {
		import delimited "${project}\fa`year'hd_data_stata.csv", clear
			keep unitid instnm addr city stabbr zip fips sector hbcu closedat
			foreach var in unitid instnm addr city stabbr zip fips sector hbcu closedat {
				tostring `var', replace
			}
		save "${project}\small`year'_directory.dta", replace
	}
...
use "${project}/small1998_directory.dta", clear
	foreach year of numlist 1999/2019 {
		merge 1:1 unitid using "${project}/small`year'_directory.dta", gen(_m`year')
	}
save "${project}/ipeds_college_entryexit_recent.dta", replace